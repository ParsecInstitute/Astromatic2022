...
name = '10k-red'
output_dir = '/home/carterrhea/Downloads'
plot_dir = '/home/carterrhea/Downloads'
...
lines_sn2 = ['OIII5007', 'OIII4959', 'Hbeta']
lines_sn1 = ['OII3726', 'OII3729']
...
def create_spectrum(spec_ct, ampls, ampls_str):
    ...
    # Redden Spectra
    Balmer_dec = np.random.uniform(2.8,6)  # Randomly select Balmer Decrement Value
    # Calculate tau value
    ...
    fluxes = [Ha_red_flux, n1_red_flux, n2_red_flux, s1_red_flux, s2_red_flux, O3_red_flux, O3_2_red_flux, Hb_red_flux, O2_red_flux, O2_2_red_flux]
    ...
    ##pickle.dump([spectrum_axis_SN3, spectrum_SN3], open('spectrum_SN3.pkl', 'a+'))
    #pickle.dump([spectrum_axis_SN2, spectrum_SN2], open('spectrum_SN2.pkl', 'a+'))
    #pickle.dump([spectrum_axis_SN1, spectrum_SN1], open('spectrum_SN1.pkl', 'a+'))
    #pickle.dump(fluxes, open('fluxes.pkl', 'a+'))
    return spectrum_axis_SN3, spectrum_SN3, spectrum_axis_SN2, spectrum_SN2, spectrum_axis_SN1, spectrum_SN1, fluxes




#for ampl, ampl_str in zip([HII_ampls, PNe_ampls, SNR_ampls],['BOND', 'PNe_2020', 'SNR_2008']):
for ampl, ampl_str in zip([HII_ampls],['BOND']):
    print("# -- Creating %s Spectra -- #"%ampl_str)
    if not os.path.exists(output_dir+ampl_str):
        os.makedirs(output_dir+ampl_str)
    spectra_axis_SN3_glob = []#np.zeros((num_syn, 685))
    spectra_SN3_glob = []#np.zeros((num_syn, 701))
    spectra_axis_SN2_glob = []#np.zeros((num_syn, 181))
    spectra_SN2_glob = []#np.zeros((num_syn, 181))
    spectra_axis_SN1_glob = []#np.zeros((num_syn, 181))
    spectra_SN1_glob = []#np.zeros((num_syn, 181))
    fluxes_global = []#np.zeros((num_syn, 10))
    for spec_ct in tqdm(range(num_syn)):
        spectrum_axis_SN3, spectrum_SN3, spectrum_axis_SN2, spectrum_SN2, spectrum_axis_SN1, spectrum_SN1, fluxes = create_spectrum(spec_ct, ampl, ampl_str)
        spectra_axis_SN3_glob.append(spectrum_axis_SN3)
        spectra_SN3_glob.append(spectrum_SN3)
        spectra_axis_SN2_glob.append(spectrum_axis_SN2)
        spectra_SN2_glob.append(spectrum_SN2)
        spectra_axis_SN1_glob.append(spectrum_axis_SN1)
        spectra_SN1_glob.append(spectrum_SN1)
        fluxes_global.append(fluxes)
    pickle.dump(spectra_SN3_glob, open('spectrum_SN3.pkl', 'wb'))
    pickle.dump(spectra_axis_SN3_glob, open('spectrum_axis_SN3.pkl', 'wb'))
    pickle.dump(spectra_SN2_glob, open('spectrum_SN2.pkl', 'wb'))
    pickle.dump(spectra_axis_SN2_glob, open('spectrum_axis_SN2.pkl', 'wb'))
    pickle.dump(spectra_SN1_glob, open('spectrum_SN1.pkl', 'wb'))
    pickle.dump(spectra_axis_SN1_glob, open('spectrum_axis_SN1.pkl', 'wb'))
    pickle.dump(fluxes_global, open('fluxes.pkl', 'wb'))
