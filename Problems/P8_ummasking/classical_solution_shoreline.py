...
def unmask_shoreline(image, mask, kernel_size=5):
    """Fills in masked areas by taking a local average. Will only fill
    pixels that are immediately adjacent to unmasked
    pixels. Iteratively fills in "shoreline" pixels until the entire
    image has been filled.
    ...
    while np.any(mimage.mask):
        ...
        DD = distance_transform_edt(
            mimage.mask == 1, return_distances=True, return_indices=False
        )
